/* 프라퍼티 어트리뷰트 */
// 객체의 프라퍼티가 생성될 때 프라퍼티에 대한 어트리뷰트(속성, 메타정보)
// 를 엔진이 내부적으로 생성
// [[Value]] : 프라퍼티의 값
// [[Writerable]] : 프라퍼티의 값을 쓸(write) 수 있는지 여부
...
// 프라퍼티 디그크립터 객체를 활용한 프라퍼티 어트리뷰트 확인
const person = {
    name: '홍길동'
};
...
// 프라퍼티는 데이터프라퍼티와 접근자프라퍼티로 구분된다.
// 데이터프라퍼티 : 값을 저장하는 프라퍼티, value를 포함
// 접근자프라퍼티 : 값을 읽거나 쓰기위한 메소드 제공, get/set 포함
const person2 = {
...
// 선언식 함수, 함수리터럴에서 new 연산자와 함께 호출하면 생성자 ㅎ마수
// 메소드, 화살표함수는 new 연산자로 호출 할 수 없는 일반 함수
...
// new로 선언식 함수를 호출하면 함수타입의 객체가 생성됨
...
// 1. 생성자함수는 return문을 사용하지 않거나 기본값을 반환하면
//    생성된 객체를 반환, 그때의 this는 생성된 객체를 가리킨다.
...
// 객체를 생성해서 반환하는 함수
// 함수에서 Object타입의 Obj객체를 반환했으므로 함수내의 this는
// Object가 된다.
function Tire2() {
    this.printTireThis = function() {
        console.log(this);
...
// ES5에서는 유사배열객체(length프라퍼티를 가진),
// ES6에서는 유사배열객체이자 이터러블(iterable)
// arguments.length : 전달받은 인자의 수
// length : 함수에 정의된 파라미터 수
function func1() {
...
// cpu 객체의 상위 객체는 Object이다.
// 결국 어떤 객체의 __proto__라는 프라퍼티는 상위객체의 prototype프라퍼티
...
// 엔진이 객체 생성시에 상속을 위해 자동으로 부여한 프라퍼티
...
// prototype은 자바스크립트에서 상속을 구현하기 위해 사용됨
// 객체에는 prototype에 접근할 수 있는 __proto__ 프라퍼티가 있다.
// 함수는 생성될 때 prototype 프라퍼티를 가지고 태어난다.
// 그래서 JS에서는 함수에 존재하는 prototype프라퍼티를 통해
// 상속을 구현하는 prototype기반의 상속 지원 언어이다!
function Diary(name){
    this.name = name;
}
...
console.log(diary.__proto__ === Object.prototype);
