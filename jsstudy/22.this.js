...
//    랙시컬하게(정적으로) 결정할 수 있는 방법을 제공하고 있다.

// 함수선언식내의 this는 함수를 호출한 객체
// f함수는 글로벌프라퍼티
function f() {
    console.log(this);
}
...
// obj는 전역프라퍼티 따라서 화살표함수 안에 있는 this는 전역객체
const obj = { // 여기 부터는 전역컨텍스트
    name: '홍길동',
    printName: function() { // 여기 부터는 객체 컨텍스트
...
// 생성자함수를 통해 생성한 객체로 호출한 함수(화살표함수포함)내의
// this는 생성된 객체
// 실행컨텍스트가 생성된 객체
function Person(name, age){ // 여기부터는 생성자 함수의 실행컨텍스트
    this.name = name;
    this.age = age;
...
// apply : 호출하는 함수내에서 rhis로 사용할 객체와 인자리스트(배열)
...
obj3.printName(); // 언디파인
obj3.printNumbers.call(obj4,3,4); // 언디파인 3 4
obj3.printNumbers.apply(obj4,[3,4]); // 언디파인 3 4
...
console.log('this' in obj);
console.log('this' in obj.printName);
...
// DOM event에서의 this바인딩
// DOM의 이벤트핸들러내에서의 this는 이벤트 타겟