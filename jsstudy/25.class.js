...
class Cat {
    ...
    // 프로토타입 메서드, prototype 객체에 등록, 객체.sayHi로 호출
    // 클래스의 프라퍼티
    sayHi() {
...
console.log(dog1.age === dog2.age); // 상속받은 정보는 각각의 객체마다 다른 주소를 가진다. 즉, 별개다.
console.log(dog1.age === Dog.prototype.age);
console.log(dog1);
...
class Phone {
    name = '폰'; // constructor 외부에 프라퍼티 만드는 건 버전 확인 요망
    get name() {
        return this.name;
    };
    set name(name) {
...
phone.name = '삼성폰'; // set name() 호출
console.log(phone.name); // get name() 호출
