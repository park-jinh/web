...
// var로 선언한 변수는 block 스코프를 가지지 않는다.
// => 즉, 블럭안에 var로 선언해도 전역변수
// ES5까지 변수 중에서 var는 전역스코프, function은 지역스코프
// ES6의 변수는 let, const: 전역스코프, function: 지역스코프,
//              Block: 지역스코프, 모듈스코프
// 스코프체인 : 식별자 검색을 위해 사용되는 하위스코프에서 상위스코프로의 연결
// JS는 모든 식별자 선언을 호이스팅 한다.
// 다만, let, const, class선언은 호이스팅을 인정하지 않는다.
...
function f3(){
    var sc_var = 'fsc_var';
    let sc_let = 'fsc_let';
    const sc_const = 'fcs_const';
    {
        var sc_var = 'bsc_var';
        let sc_let = 'bsc_let';
        const sc_const = 'bcs_const';
        console.log(sc_var); // bsc
        console.log(sc_let); // bsc
        console.log(sc_const); // bsc
    }
    console.log();
    console.log(sc_var); // bsc
    console.log(sc_let); // fsc
    console.log(sc_const); // fsc
}
f3();
console.log();
console.log(sc_var); // gsc
console.log(sc_let); // gsc
console.log(sc_const); //gsc
...
// 왜냐하면 재정의된 함수 두개는 코드실행전에 호이스팅된다.
f4();
function f4() {
    console.log('f4');
}
...
function f5(){
    console.log(ex_var); // 1.undefined 2.undefined
    console.log(ex_let); // 1.ex_let    2.error
    ex_var = 'f5_ex_var';
    ex_let = 'f5_ex_let';
    var ex_var; // 1. 현재라인 주석해제시 결과 예측
    // let ex_let; // 2. 현재라인 주석해제시 결과 예측
}
f5();
