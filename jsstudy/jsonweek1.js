...
$(function() {
    ...
    let commentDiv = $('<div id="cmtDiv"></div>')
        ...
        $.get({
            ...
            success: function(result) {
                result.forEach(element => {
                    $('tbody').append(
                        $('<tr></tr>')
                            ...
                            .append($(`<td>${usernameMap[element.userId]}</td>`))
                            .append($(`<td>${element.title}</td>`))
                            .append($(`<td></td>`).append($(`<input type="button" class="cmtBtn" name="${element.id}" value="댓글보기">`)))
                    );
                });
                $('.cmtBtn').on('click',function(){
                    ...
                    commentDiv.empty();
                    $.get({
                        url: 'https://jsonplaceholder.typicode.com/comments?postId=' + $(this).prop('name'),
                        success: function(result){
                            result.forEach(item=>{
                                commentDiv.append(`<p>번호: ${item.id}, 이름: ${item.name}, 내용: ${item.body}</p>`);
                            });
                        }
                    });
                    $('div').after(commentDiv);
                });
            }
        });
    });
